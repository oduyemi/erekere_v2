...
@starter.route("/tours", methods=["GET", "POST"], strict_slashes = False)
def tours():
    tour = Tour.query.order_by(Tour.tour_id.desc()).all()
    if request.method == "POST":
        req = request.form
        tname = req.get("tname")
        tprice = req.get("tprice")
        tour_id =  tour[0].tour_id
        startdate = tour[0].tour_startdate.strftime("%Y-%m-%d")
        reg = Trip(trip_tour = tour_id, trip_payment_status = 1)
        db.session.add(reg)
        db.session.commit()
        whatsapp_message = f"Hi! I would like to book {tname} that will start by {startdate} for ₦{tprice}."
        whatsapp_link = f"https://api.whatsapp.com/send?phone=2348166336187&text={whatsapp_message}"
